...
g++ -Wall -o S10E3 S10E3.cpp `pkg-config --cflags --libs opencv`
...
int main(void)
{
	...
	Rect reg2b2(Point(190,150),Size(40,40));
	...
	Rect regcamara(Point(30,80), Size(150,150));
	...
	for(;;)
	{
		...
		switch(V){
		  ...
		  menu_act.copyTo(base(reg2));
			...
			cvtColor(	imagen_reducida,
								imagen_reducida,
								CV_BGR2GRAY);
			imwrite("gris.png", imagen_reducida);
		 	imagen_hsv = imread("gris.png");
			imagen_hsv.copyTo(base(regcamara));
			...
			switch(Op){
						...
						case 2:
							...
							imwrite("Imagen.png", imagen_reducida);
								break;
