...
g++ -Wall -o S5E3 S5E3.cpp `pkg-config --cflags --libs opencv`
		...
		if( x > 0 && x < 250 && y > 0 && y<250)
			{
				...
				//SePrecionoBoton = 1;
	cout << "La variable vale " << SePrecionoBoton << endl;
			SePrecionoBoton = SePrecionoBoton+1;
			if(SePrecionoBoton>2) SePrecionoBoton = 0;
...
int main(void)
{
	...
	Mat D(250,250,CV_8UC3,Scalar(25, 75, 12)); 
	Mat E(250,250,CV_8UC3,Scalar(10, 142, 100)); 
	...
	for(;;)
	{
		switch(SePrecionoBoton)
		{
			case 0: B.copyTo(A(region1)); break;
			case 1: D.copyTo(A(region1)); break;
			case 2: E.copyTo(A(region1)); break;
			default: C.copyTo(A(region1)); 
